...
void ConvertDirectorySL_Raw(string dirname, TString run="0", bool zerosupression=false, TString outputname="default", int jinitial=0)
{

  std::cout << "dirname " << dirname << " Run: " << run << std::endl; 

  for(int j=jinitial; j<5000; j+=1) {
    //for(int j=0; j<1; j++) {
    
    TString filen="_raw.bin";
    if(j==0) filen=TString::Format("%s_raw.bin",run.Data());
    ...
    if(outputname!="default") output=TString::Format("%s/converted_%s.root",dirname.c_str(),filen.Data());
    
    vector<TString>* filenames = list_files(dirname.c_str(),filen.Data());
    if(filenames->size()!=1) {
      std::cout << "N files != 1 !!!  for "<< filen << std::endl;
      break;
    }
    ...
    while(result==false) {
      result=ss.ReadFile((*filenames)[0], true, output, zerosupression, true);
      ss._maxReadOutCycleJump*=10;
    }
    //    delete ss;
